  ...
  return (
    <main style={page}>
      <nav style={nav}>
        ...
        <div style={navLinks}>
          <button onClick={() => scrollTo("audit")} style={navLink}>AI Audit</button>
          <button onClick={() => scrollTo("agents")} style={navLink}>Agents</button>
          <button onClick={() => scrollTo("security")} style={navLink}>Security</button>
          <button onClick={() => scrollTo("roi")} style={navLink}>ROI</button>
        </div>
        <button onClick={() => scrollTo("audit")} style={outlineButton}>Request Demo</button>
      </nav>
      ...
      <section style={hero}>
        <div>
          ...
          <div style={buttonRow}>
            <button style={primaryButton} onClick={() => { setReportReady(true); scrollTo("report"); }}>Run AI Savings Audit</button>
            <button style={secondaryButton} onClick={() => scrollTo("agents")}>See What Agents Do</button>
          </div>
        ...
        <div style={heroPanel}>
          ...
          <div style={radarWrap}>
            ...
            {agents.slice(0, 4).map((agent, index) => (
              <button key={agent.name} onClick={() => { setActiveAgent(agent); scrollTo("agents"); }} style={{ ...agentDot, ...dotPositions[index] }}>{agent.name.split(" ")[0]}</button>
            ))}
          </div>
      ...
      <section id="audit" style={twoCol}>
        <div style={cardBlack}>
          ...
          <button style={whiteButton} onClick={() => { setReportReady(true); scrollTo("report"); }}>Generate Audit Report</button>
        </div>

        <div id="agents" style={cardWhite}>
          <h2>Click an AI Agent</h2>
          <div style={agentGrid}>{agents.map((agent) => <button key={agent.name} onClick={() => setActiveAgent(agent)} style={agent.name === activeAgent.name ? activeAgentCard : agentCard}><strong>{agent.name}</strong><p>{agent.role}</p></button>)}</div>
        </div>
      </section>
      ...
      <section style={agentDemo}>
        ...
        <div><h3>What this agent does</h3><ul>{activeAgent.actions.map((action) => <li key={action}>{action}</li>)}</ul><button style={primaryButton} onClick={() => setReportReady(true)}>Add to Client Proposal</button></div>
      </section>
      ...
      <section id="security" style={securityBand}><div><h2>Security promise for every company upload.</h2><p>No selling customer data. Clear data controls. Optional human review. Delete reports and files on request.</p></div><button style={whiteButton}>View Privacy Policy</button></section>
    </main>
  );
}
